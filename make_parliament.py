...
YEAR_SEATS = {2019: 338, 2021: 338, 2011: 308}
PARTY_NAMES = [
    'Liberal', 'Conservative', 'NDP-New Democratic Party', 'Green Party',
    'VCP', "People's Party", 'Christian Heritage Party', 'Independent',
    'National Citizens Alliance', 'Communist', 'Animal Protection Party',
    'Libertarian', 'Parti Rhinocéros Party', 'Bloc Québécois',
    'Radical Marijuana', "l'Indépendance du Québec", 'ML', 'PC Party',
    'Nationalist', "CFF - Canada's Fourth Front", 'UPC', 'Stop Climate Change',
    'Free Party Canada', 'Parti Patriote', 'Maverick Party', 'Centrist',
    'Pirate Party', 'CAP', 'United Party', 'FPNP',
]
TRANSLATE_NAMES = {
    "People's Party - PPC": "People's Party", 'Marxist-Leninist': 'ML',
    'Marijuana Party': 'Radical Marijuana',
    "Pour l'Indépendance du Québec": "l'Indépendance du Québec",
    'No Affiliation': 'Independent', 'CHP Canada': 'Christian Heritage Party',
    'Bloc Quebecois': 'Bloc Québécois', 'Rhinoceros': 'Parti Rhinocéros Party',
    'Canadian Action Party': 'CAP',
    'Animal Alliance': 'Animal Protection Party', 'WBP': 'Maverick Party'
}
...
def assign_seats(doc, num_seats, SEATS_PER_AISLE, year, curr_parties,
                 party_winners, rep_status):
  ...
  # curr_parties = curr_parties[:1]
  last_aisle_unplaced = True
  for party in curr_parties:
    winning_party = party == curr_parties[0]
    for winner in party_winners[party]:
      offset = 0
      threshold = ((row_count-1)*SEATS_PER_AISLE)+last_row_seat_count/2
      if ((winning_party and gov_seat_count >= threshold) or
              (not winning_party and opp_seat_count >= threshold)):
      ...
      if last_aisle_unplaced and offset > 0:
        last_aisle_unplaced = False
        if doc.select_one(f"#{row_id} .aisle") is None:
      ...
      if (
          ...
          len(doc.find(id=row_id).find_all(recursive=False)) == 0
      ):
        blank = doc.new_tag('td')
        doc.find(id=row_id).append(blank)
...
def make_parliament(doc, candidates, year):
  ...
  for candidate in candidates:
    (district_name, district_num, name, party, votes, name_and_party) = (
        candidate['district_name'], candidate['district_num'],
        candidate['name'], candidate['party'], candidate['votes'],
        candidate['name_and_party'])
    votes = int(votes)
    if district_num not in districts:
      ...
      local_cand_votes = districts[district_num]['candidate_votes']
      local_winner = districts[district_num]['winner']
      local_cand_votes[name_and_party] = votes
      districts[district_num]['total_votes'] += votes
      if votes > local_cand_votes[local_winner]:
        districts[district_num]['runner_up'] = local_winner
        districts[district_num]['winner'] = name_and_party
      elif (districts[district_num]['runner_up'] == 'none' or
            votes > local_cand_votes[districts[district_num]['runner_up']]):
  ...
  for candidate in candidates:
    ...
    if name_and_party == districts[district_num]['winner']:
      local_runner_up = districts[district_num]['runner_up']
      rep_votes = districts[district_num]['candidate_votes'][local_runner_up]+1
    ...
    if party not in party_candidate_vote:
      party_candidate_vote[party] = {
          name: {'rep_votes': rep_votes, 'unrep_votes': unrep_votes}}
      party_total_vote[party] = {
          'rep_votes': rep_votes, 'unrep_votes': unrep_votes}
    else:
      party_candidate_vote[party][name] = {
          'rep_votes': rep_votes, 'unrep_votes': unrep_votes}
      party_total_vote[party]['rep_votes'] += rep_votes
      party_total_vote[party]['unrep_votes'] += unrep_votes

  seats_in_unrep_parliament = round(
      YEAR_SEATS[year]*total_unrep_votes/total_rep_votes)
  print(year, total_rep_votes/YEAR_SEATS[year], 'votes/elected member')
  new_seat_count = 0

  for party in curr_parties:
    party_total_vote[party]['seats'] = round(
        seats_in_unrep_parliament *
        party_total_vote[party]['unrep_votes'] /
        total_unrep_votes)
    ...
    # 'unrep seats=',round(seats_in_unrep_parliament*
    # party_total_vote[party]['unrep_votes']/total_unrep_votes))
    ...
    for candidate in list(party_candidate_vote[party]):
      ...
      if x[2]:
        party_rep_winners[party][candidate] = [
            candidate,
            party_candidate_vote[party][candidate]['rep_votes'],
            False
        ]
    party_unrep_winners[party] = sorted(
        party_unrep_winners[party],
        key=lambda candidate:
        candidate[1]*-1)[:party_total_vote[party]['seats']]
    ...
    party_rep_winners[party] = sorted(
        party_rep_winners[party], key=lambda candidate: candidate[1]*-1)
  ...
  curr_unrep_parties = sorted(
      curr_parties, key=lambda party: -len(party_unrep_winners[party]))
  curr_rep_parties = sorted(
      curr_parties, key=lambda party: -len(party_rep_winners[party]))
  # print(curr_parties)
  year_div = BeautifulSoup(make_year_html(year), 'html.parser')
  doc.find(class_='vertical').append(year_div)
  doc = assign_seats(doc, seats_in_unrep_parliament, SEATS_PER_AISLE, year,
                     curr_unrep_parties, party_unrep_winners, 'unrepresented')
  doc = assign_seats(doc, YEAR_SEATS[year], SEATS_PER_AISLE//2, year,
                     curr_rep_parties, party_rep_winners, 'represented')
  return doc


def parse_csv_format1(candidates):
  consistently_formatted = []
  # district_name, district_num, name_and_party, votes
  for candidate in candidates:
    (prov, district_name, district_num, name_and_party, residence,
     occupation, votes, vote_per, maj, maj_per) = candidate
    c = {}
    found = False
    ...
    consistently_formatted += [c]
  return consistently_formatted
...
for candidate in candidates:
  (
      district_num, district_name, _, result_type, _, surname, middle_name,
      given_name, party, _, votes, vote_per, votes_rejected, total_ballots
  ) = candidate
...
with open('2011 results by candidate.csv', "r", newline='') as csvfile:
  candidates = csv.reader(csvfile, delimiter=',')
  candidates = list(candidates)[1:]
candidates_2011 = parse_csv_format1(candidates)
doc = make_parliament(doc, candidates_2011, 2011)
